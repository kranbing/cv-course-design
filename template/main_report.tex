...
需要说明的是，本文当前已完成的实验只关注不同方法在同一数据集、同一划分、同一训练设置下的横向性能对比，暂不展开复现细节实验。消融实验仍属于课程设计的后续工作，将由小组另一位成员继续补充。由于课程设计环境和时间有限，输入图像统一缩放到$96 \times 128$，该设置主要用于保证不同方法可以在本机完成可重复训练与测试。因此，本实验属于完整CamVid划分下的轻量benchmark对比，绝对指标不直接等同于高分辨率、长周期训练的公开排行榜结果。
...
本次实验保存了不同方法在测试/验证集上的预测可视化结果。从预测结果观察，UNetSmall对道路、天空、建筑等大面积区域较稳定；TinySegFormer能够利用全局上下文改善部分区域一致性，但在低分辨率训练条件下容易出现边界粗糙；TinySegFormer\_Boundary在部分车辆轮廓和道路边缘处的预测更连贯。由于当前阶段重点是方法横向对比，本文暂不展开额外的复现细节分析。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{camvid_prediction_gallery.png}
\caption{CamVid测试/验证集上不同方法的预测结果可视化对比}
\label{fig:camvid_prediction_gallery}
\end{figure}

图\ref{fig:camvid_prediction_gallery}给出了相同样本下UNetSmall、TinySegFormer和TinySegFormer\_Boundary的预测掩码。三种方法都能恢复道路、天空和建筑等主体区域，但在车辆轮廓、远处行人和道路标线等细节区域存在明显差异。边界增强模型在部分样例中对车体边缘和道路边缘的连贯性更好，这与Boundary F1的定量变化相互印证。

\subsection{消融实验预留}

为进一步验证边界增强设计中各组成部分的作用，后续消融实验将由小组另一位成员完成。本节预留消融实验位置，计划在保持完整CamVid划分、训练轮数、输入分辨率、优化器和评价指标一致的条件下，围绕TinySegFormer\_Boundary展开组件级比较。

后续建议补充三组实验。第一组比较是否加入边界监督，即TinySegFormer与TinySegFormer\_Boundary的直接对比，用于判断边界分支对mIoU、PA和Boundary F1的整体影响。第二组比较不同边界损失权重$\lambda$，例如$\lambda=0.1$、$\lambda=0.3$和$\lambda=0.5$，用于分析边界监督强度过弱或过强时对主分割任务的影响。第三组比较边界监督只参与训练和边界预测同时参与推理两种设置，用于判断边界分支主要提供训练约束，还是能够在推理阶段进一步改善分割轮廓。
...
\toprule
实验设置 & mIoU / \% & PA / \% & Boundary F1 / \% & 说明 \\
\midrule
TinySegFormer & 待补充 & 待补充 & 待补充 & 无边界监督 \\
Boundary, $\lambda=0.1$ & 待补充 & 待补充 & 待补充 & 弱边界监督 \\
Boundary, $\lambda=0.3$ & 待补充 & 待补充 & 待补充 & 默认设置 \\
Boundary, $\lambda=0.5$ & 待补充 & 待补充 & 待补充 & 强边界监督 \\
\bottomrule
\end{tabularx}
\end{table}

完成消融实验后，需要在本节补充两类分析：一是定量分析不同设置下mIoU、PA和Boundary F1的变化，说明边界监督是否真正带来稳定增益；二是结合可视化结果讨论边界增强对道路边缘、车辆轮廓、行人等小目标或细长结构的影响。需要明确区分已有方法和本文设计：TinySegFormer来自对SegFormer思想的轻量化实现，边界监督分支和损失权重实验属于本文在课程设计中的改进设想与验证内容。
\section{总结}
